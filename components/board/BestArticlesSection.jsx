...
const BestArticleCard = ({ article }) => {
  const dateString = format(new Date(article.createdAt), "yyyy. MM. dd"); // Date 객체로 변환 필요
...
const BestArticlesSection = () => {
  ...
  const [pageSize, setPageSize] = useState(null); // 초기 값을 실제 사용되는 값인 1 또는 3으로 설정해도 되지만, 이 경우에는 화면 크기가 파악되기 전에는 pageSize가 설정되지 않았음을 명확히 하기 위해 null로 두었어요.
      ...
      const fetchBestArticles = async (size) => {
        try {
          const response = await fetch(
            `${process.env.NEXT_PUBLIC_API_BASE_URL}/articles?orderBy=recent&take=${size}`
          );

          const data = await response.json();
          setArticles(data.data || []); // API 변경에 맞춰 data 구조를 사용
        } catch (error) {
